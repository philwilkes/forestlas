...
class canopyStructure:
    ...
    def __init__(self, las_files, alpha=.3, N=10, resolution=None,
       ...
       and not point_only:
        ...
        if self.verbose:
            print 'number of tiles to process:', len(self.LASlist)
            print 'number of CPUs:', self.number_of_processes
    ...
    def mp(self, listItems):
        ...
        p = multiprocessing.Pool(self.number_of_processes)
        # [p.apply(recover_temp_file, (f,), kw) for f in F]
        [p.apply(readLAS, (self, f)) for f in listItems]
        p.close()
        p.join()
...
if __name__ == '__main__':
    ...
    canopyStructure(L, pgap=True, verbose=True, number_of_processes=4, resolution=5).exportTiff(path)
