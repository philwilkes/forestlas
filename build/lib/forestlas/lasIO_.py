...
class lasIO:
    ...
    #         for i in sample: # loops through all points
    #
    #             if self.Global.totPoints%self.counter == 0 and self.verbose:
    #                 print "{:.0f}% | {} of {} new tiles created | {} | {}".format((self.Global.totPoints / float(self.globalHeader['numptrecords'])) * 100,
    ...
    def all(self, take_sample=False):
        ...
        self._process_las('all')
        return self

    def _process_all(self, tile):
        ...
        with open(os.path.join(self.dir, tile), 'rb') as fh:
            ...
            for i in sample: # loops through all points
                ...
                if self.Global.totPoints%self.counter == 0 and self.verbose:
                    print "{:.0f}% | {} of {} points processed | {} | {}".format((self.Global.totPoints / float(self.globalHeader['numptrecords'])) * 100,
                                                                             self.Global.totPoints, self.globalHeader['numptrecords'],
                                                                             tile,
                                                                             datetime.datetime.now())

                fh.seek(h["offset"] + (i * h['pointreclen'])) # searches to beginning of point
                self.xy_dictionary["all"] = self.writePoint(self.xy_dictionary["all"], extract_return(self, fh), h)
                self.Global.totPoints += 1
    ...
    def _process_las(self, method):
        ...
        if method == 'all':
            ...
            if self.verbose: print 'as using all method number of processes changed to 1'

        for i in range((len(self.tile_list) / self.number_of_processes) + 1):

            jobs = []

            if (self.number_of_processes * loops) + self.number_of_processes < len(self.tile_list):
                processingList = self.tile_list[self.number_of_processes * loops: (self.number_of_processes * loops) + self.number_of_processes]
            else:   processingList = self.tile_list[self.number_of_processes * loops:]

            for j, las in enumerate(processingList): # limits number of lass run at once

                # if target_args == 'all':
                method_dic = {'all': self._process_all,
                              'tile': self._process_tile,
        ...
        if method == 'all':
            self.xy_dictionary['all']['i'] = self.Global.totPoints
